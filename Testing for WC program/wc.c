...
wc (int fd, char const *file_x, struct fstatus *fstatus)
{
  ...
  if (count_bytes && !count_chars && !print_lines && !count_complicated)
  ...
  else if (!count_chars && !count_complicated)
    {
      ...
      [[loop_target]]while ((bytes_read = safe_read (fd, buf, BUFFER_SIZE)) > 0)
        {
          char *p = buf;
...
int main(int argc, char **argv){
  ...
  char* temp[4];
  temp[0] = "wc";
  temp[1] = "-l"; // Make sure the target loop is hit
  temp[2] = "temp_wc.txt";
  temp[3] = NULL;
  FILE *temp_input = fopen("temp_wc.txt", "w+");
  if(temp_input == NULL){
    return EXIT_FAILURE;
  }
  for(i = 0; i < atoi(argv[1]); ++i){
    for (j = 0; j < BUFFER_SIZE/8; ++j) {
      fprintf(temp_input, "deadbeef");
    }
  }
  fclose(temp_input);
  temp_main(3, temp);
